...
func PeriodeHome(c *fiber.Ctx) error {
	...
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company)
...
func PeriodeDetail(c *fiber.Ctx) error {
	...
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company) + "_INVOICE_" + strconv.Itoa(client.Idtrxkeluaran)
	render_page := time.Now()
...
func PeriodeListMember(c *fiber.Ctx) error {
	...
	temp_decp := helpers.Decryption(name)
	_, client_company, _, _ := helpers.Parsing_Decry(temp_decp, "==")
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company) + "_INVOICE_" + strconv.Itoa(client.Idtrxkeluaran) + "_LISTMEMBER"
	render_page := time.Now()
...
func PeriodeListBet(c *fiber.Ctx) error {
	...
	temp_decp := helpers.Decryption(name)
	_, client_company, _, _ := helpers.Parsing_Decry(temp_decp, "==")
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company) + "_INVOICE_" + strconv.Itoa(client.Idtrxkeluaran) + "_LISTPERMAINAN_" + client.Permainan
	render_page := time.Now()
...
func PeriodeListBetstatus(c *fiber.Ctx) error {
	...
	temp_decp := helpers.Decryption(name)
	_, client_company, _, _ := helpers.Parsing_Decry(temp_decp, "==")
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company) + "_INVOICE_" + strconv.Itoa(client.Idtrxkeluaran) + "_LISTBET_" + client.Status
	render_page := time.Now()
	var obj responseredis_periodelistbet
	var arraobj []responseredis_periodelistbet
	resultredis, flag := helpers.GetRedis(field_redis)
...
func PeriodeListBetTable(c *fiber.Ctx) error {
	...
	temp_decp := helpers.Decryption(name)
	_, client_company, _, _ := helpers.Parsing_Decry(temp_decp, "==")
	field_redis := Fieldperiode_home_redis + strings.ToLower(client_company) + "_INVOICE_" + strconv.Itoa(client.Idtrxkeluaran) + "_LISTBETTABLE"
	render_page := time.Now()
...
func PeriodeSaveNew(c *fiber.Ctx) error {
	...
	if typeadmin == "MASTER" {
		...
		_deleteredis_periode(client_company, 0, idpasarantogel)
		return c.JSON(result)
	} else {
		if !flag_page {
			c.Status(fiber.StatusForbidden)
...
func PeriodeSaveRevisi(c *fiber.Ctx) error {
	...
	if typeadmin == "MASTER" {
		...
		_deleteredis_periode(client_company, client.Idinvoice, idpasarantogel)
		return c.JSON(result)
	} else {
		if !flag_page {
			c.Status(fiber.StatusForbidden)
...
func _deleteredis_periode(company string, idtrxkeluaran int, idpasarantogel string) {
	...
	//FRONTEND
	val_frontend := helpers.DeleteRedis("listresult_" + strings.ToLower(company) + "_" + strings.ToLower(idpasarantogel))
	val_frontend_listpasaran := helpers.DeleteRedis("listpasaran_" + strings.ToLower(company))
	val_frontend_listresult := helpers.DeleteRedis("listresult_" + strings.ToLower(company))
